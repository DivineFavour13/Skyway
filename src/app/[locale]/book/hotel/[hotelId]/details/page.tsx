...
export default function HotelDetailsPage() {
  ...
  const {
    selectedHotel, selectedRoom, setSelectedRoom,
    checkin, checkout, guests, guestDetails, setGuestDetails,
  } = useHotelStore();
                    ...
                    <div className="flex-1">
                      ...
                      <div className="flex flex-wrap gap-1 mt-2">
                        {room.amenities.map((a) => (
                          <span key={a} className="text-[10px] px-1.5 py-0.5 rounded bg-[var(--color-surface-raised)] text-[var(--color-text-muted)]">
                            {a}
                          </span>
                        ))}
                      </div>
...
function inputClass(hasError: boolean) {
  return `w-full h-11 px-4 rounded-xl border bg-[var(--color-surface)] text-sm text-[var(--color-text-primary)] placeholder:text-[var(--color-text-muted)] outline-none transition-colors ${
    hasError ? 'border-[var(--color-error)]' : 'border-[var(--color-border)] focus:border-[var(--color-accent)]'
  }`;
}