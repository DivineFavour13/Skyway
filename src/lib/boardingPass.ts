...
export function generateBoardingPass(
  offer: DuffelOffer,
  passengers: PassengerDetails[] | PassengerDetails,
  ref: string,
  seatDesignators: string[] = []
) {
  const passengerList = Array.isArray(passengers) ? passengers : [passengers];
  ...
  passengerList.forEach((passenger, index) => {
    ...
    doc.setFont('helvetica', 'bold');
    doc.setFontSize(18);
    doc.text(seatDesignators[index] || '—', 152, 60);
